...
# Creamos un "cliente" de API. Si la clave no existe, el cliente no se crea.
client_openai = None
api_key = os.environ.get('OPENAI_API_KEY')
if api_key:
    client_openai = OpenAI(api_key=api_key)
else:
    logger.warning("No se encontró API Key de OpenAI. La funcionalidad de IA estará deshabilitada.")
...
def obtener_respuesta_del_agente(mensaje_usuario: str, cliente: Cliente, conversacion: Conversacion):
    """
    El "cerebro" del bot con manejo mejorado de errores
    """
    ...
    try:
        ...
        if pregunta_coincidente:
            logger.info(f"📋 Respondiendo con PreguntaFrecuente ID: {pregunta_coincidente.pregunta_frecuenta_id}")
            return crear_respuesta_texto(pregunta_coincidente.respuesta)

        # Buscar en tipos de habitación
        q_habitaciones = Q()
        for palabra in palabras:
        ...
        if habitacion_coincidente:
            logger.info(f"🏨 Encontrada coincidencia con TipoHabitacion: {habitacion_coincidente.nombre_tipo_habitacion}")
            respuesta_texto = (
    ...
    # --- CAPA 2: INTENTO DE CONEXIÓN CON LA IA ---
    if client_openai:
        try:
            logger.info(f"🤖 Consultando a OpenAI para la conversación {conversacion.conversacion_id}...")
            ...
            instrucciones_sistema = (
                "Eres Pratsy, un asistente virtual amigable para Moteles Prats. "
                "Ayudas a los clientes con información sobre habitaciones, precios y servicios. "
                "Responde de forma concisa, amigable y profesional. "
                "Si no sabes algo específico, ofrece ayuda general."
            )
            
            # Preparar mensajes para la API
            mensajes_para_api = [{"role": "system", "content": instrucciones_sistema}]
            for msg in historial_mensajes:
                role = "user" if msg.remitente == 'cliente' else "assistant"
                mensajes_para_api.append({"role": role, "content": msg.contenido})
            
            # Llamada a OpenAI con manejo específico de errores
            ...
            logger.info(f"✅ Respuesta recibida de OpenAI: {respuesta_ia}")
            return crear_respuesta_texto(respuesta_ia)
            
        except Exception as e:
            # Log específico del tipo de error y continuar al respaldo
            error_msg = str(e)
            if "429" in error_msg or "insufficient_quota" in error_msg:
                logger.warning(f"💳 OpenAI sin cuota disponible - Usando respaldo con preguntas frecuentes")
                logger.info(f"🔄 Saltando a capa de respaldo...")
            elif "timeout" in error_msg.lower():
                logger.warning(f"⏰ Timeout en OpenAI - Usando respaldo")
                logger.info(f"🔄 Saltando a capa de respaldo...")
            else:
                logger.warning(f"🤖 Error en OpenAI - Usando respaldo: {e}")
                logger.info(f"🔄 Saltando a capa de respaldo...")
            
            # Explícitamente pasar a la capa de respaldo
            pass

            try:
                # Evitar duplicados recientes del mismo cliente
                if not PreguntaDesconocida.objects.filter(cliente=cliente, texto_pregunta=mensaje_usuario).exists():
                    ...
                    logger.info(f"📥 Pregunta desconocida guardada para revisión: '{mensaje_usuario}'")
            except Exception as e:
                logger.error(f"❌ Error guardando pregunta desconocida: {e}")

            logger.info("🔄 Usando respuesta de respaldo (sin IA) - Creando botones con preguntas frecuentes")
            return crear_respuesta_botones()
    # --- CAPA 3: RESPUESTA DE RESPALDO ---
    logger.info("🔄 Usando respuesta de respaldo (sin IA) - Creando botones con preguntas frecuentes")
    respuesta_respaldo = crear_respuesta_botones()
    logger.info(f"📋 Respuesta de respaldo creada: {json.dumps(respuesta_respaldo, indent=2, ensure_ascii=False)}")
